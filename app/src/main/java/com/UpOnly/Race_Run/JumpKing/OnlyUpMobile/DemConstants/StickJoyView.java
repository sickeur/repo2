...
public class StickJoyView extends View implements Runnable {
    // Constants
    ...
    public StickJoyView(Context context) {
        super(context);
    }

    public StickJoyView(Context context, AttributeSet attrs) {
        super(context, attrs);
        InitStickJoyView();
    }

    public StickJoyView(Context context, AttributeSet attrs, int defaultStyle) {
        super(context, attrs, defaultStyle);
        InitStickJoyView();
    }

    protected void InitStickJoyView() {
        mainCircle = new Paint(Paint.ANTI_ALIAS_FLAG);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            mainCircle.setColor(getContext().getColor(R.color.twhite));
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // height
        int d = Math.min(DuhMeasurment(widthMeasureSpec), DuhMeasurment(heightMeasureSpec));

        setMeasuredDimension(d, d);

    }

    private int DuhMeasurment(int measureSpec) {
        int result = 0;
...
//                (float) centerX, (float) centerY, horizontalLine);
    ...
    public void StickJoyMoveListener(OnJoystickMoveListener listener,
                                     long repeatInterval) {
